...
# copy trained encoder weights into combined model
combined_model.get_layer('encoder').set_weights(encoder.get_weights())

# generate embeddings and initialize cluster centers
...
# ── SwarmCallback — the 4 lines that enable federation ────────────────────
try:
    ...
    # running locally without Swarm — skip callback
    callbacks = []
...
for epoch in range(max_epochs):
    ...
    h = combined_model.fit(
        ...
        batch_size=batch_size,
        verbose=0,
        callbacks=callbacks    # SwarmCallback syncs weights here
    )
